...
- **Genre / Length / Tempo / Swing** — shape the current loop. Genre & length are "new song" changes
  (they land cleanly on the beat / at the loop end); tempo & swing change live like knobs.
...
Each voice (drums, perc, bass, lead, synth, accents, fills, FX) is a module with:
- a **sound selector** and a **level**,
- **dice** (re-roll that voice's sound), **reverb** (add space to that voice), **kill** (mute it).
The **FX** voice makes risers, glitches, impacts, sweeps, and a ship horn.
**MASTER FX** (top-right of Voices) adds reverb to the whole mix (except drums) — with a strength dial.
