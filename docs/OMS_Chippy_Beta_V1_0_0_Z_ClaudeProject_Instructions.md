# OMS Chippy [Beta V1_0_0] — CLAUDE PROJECT INSTRUCTIONS
...
- **Single-file, no dependencies, no build.** Never split, never add a framework, package, or bundled library. (An MP3 export using inlined lamejs was tried in V0_3_1 and reverted in V1_0_0 to keep this principle; export is native WAV.)
...
- Iterations at the current minor (e.g. V0_1_0 → V1_0_0); bug fixes edit in place, features bump the patch letter/number.
- Deliverables packages wrap an iteration series into the next version. (V1_0_0 is the debut package — build and package number aligned; the wrap-into-next cadence starts after.)
